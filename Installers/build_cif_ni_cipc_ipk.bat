@echo off
rem Windows launcher for the cif_ni_cipc_ipk build.
rem
rem Requires WSL because the IPK is assembled with Linux tar and bash scripts.
rem Converts the repo path to a WSL path, then runs build_cif_ni_cipc_ipk.sh.
rem Output is written to Installers\output\cif-ni-cipc_<version>_<arch>.ipk
setlocal
...
where wsl >nul 2>&1
if errorlevel 1 (
  echo Error: WSL is required to build the cif_ni_cipc_ipk package.
  exit /b 1
)

rem Translate the Windows repo path to a path WSL can use, e.g. /mnt/d/dev/Packages/NI_CIPC
