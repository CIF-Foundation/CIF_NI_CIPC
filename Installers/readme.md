...
Copy the installers directory to the home directory <cp -r  /mnt/d/dev/Packages/NI_CIPC/Installers/ ~/installers>
Run the builds <bash ~/installers/build_deb.sh>  <bash ~/installers/build_ipk.sh>
Copy the files back to a mounted location <cp -r  ~/installers/cif-ni-cipc.0.1.1.0.ipk /mnt/d/dev/builds/installers/>
